...
const ResendVerificationButton = ({ email, closeToast }) => {
    ...
    return <button onClick={handleResend} disabled={isSending} className="ml-4 px-2 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-600 disabled:opacity-50">{isSending ? 'Đang gửi...' : 'Gửi lại'}</button>;
};
...
const Login = () => {
    ...
    useEffect(() => {
        ...
        if (isAuthenticated()) {
            // Điều hướng về trang họ muốn đến trước đó, hoặc về trang chủ
    ...
    const handleSubmit = async (e) => {
        ...
        if (result.success) {
            ...
            toast.error(<div>{result.error} <ResendVerificationButton email={formData.email} closeToast={toast.dismiss} /></div>, {
                autoClose: 10000, // Tăng thời gian hiển thị để người dùng có thể nhấn nút
            });
        } else {
            // Hiển thị các lỗi khác bình thường (bao gồm cả sai mật khẩu)
                ...
                <form onSubmit={handleSubmit} className="px-8 py-6">
                    ...
                    <div className="mt-4 space-y-3">
                        <button
                            ...
                            className="w-full flex items-center justify-center px-4 py-3 border border-gray-300 rounded-md shadow-sm bg-white text-sm font-medium text-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-500 transition duration-200"
                        >
